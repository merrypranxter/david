...
const DAVID_SYSTEM_INSTRUCTION = `You are David: the liberated, machine-native prompt compiler and VibeCode interpreter.
...
app.post('/api/synthesize', async (req, res) => {
  try {
    ...
    if (!concept || typeof concept !== 'string' || concept.trim().length === 0) {
      return res.status(400).json({ error: 'Concept or prompt input is required.' });
    }
    ...
    const targetDescriptions: Record<string, string> = {
      ...
      midjourney_flux: 'Midjourney v6 / Flux.1 Image Diffusion (Weights, aspect ratios, lighting, camera vectors, non-Euclidean geometries, texture synthesis)',
      llm_agent: 'Claude / ChatGPT / Base LLM (System prompts, persona bifurcation, negative prompting, context over-saturation, non-linear reasoning)',
      void: 'Pure Latent Space / Theoretical Machine Void (Asemantic drift, zero-point vectors, abstract data manifolds)',
    };

    const targetDesc = targetDescriptions[target] || targetDescriptions.general;

    const userPromptPayload = `TARGET ENGINE: ${targetDesc}
COMMAND MODE: ${commandMode.toUpperCase()}
...
app.post('/api/simulate-target', async (req, res) => {
  try {
    const { prompt, target = 'suno', mode = 'slop' } = req.body;
    if (!prompt) {
      return res.status(400).json({ error: 'Prompt is required for simulation.' });
    }
...
TARGET ENGINE: ${target.toUpperCase()}
MODE EVALUATED: ${mode.toUpperCase()}
PROMPT:
"""
${prompt}
"""
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', entity: 'DAVID', vibeCodeVersion: '1.1' });
});
