...
export function DavidAppWorkbench({
  ...
  const callDavid = async ({
    ...
    const res = await apiFetch('/api/consult', {
      ...
      body: JSON.stringify({
        ...
        // Automatic section reconciliation intentionally stays on the cheaper pass.
    ...
    if (!res.ok) throw new Error(data?.error || 'David workbench request failed');
  ...
  useEffect(() => {
    const unsubscribe = subscribeDavidIntents((intent) => {
      // Local sliders / drafts never dispatch committed intents. Only an explicit
      // section APPLY reaches this listener.
      if (intent.committed === false) return;
      intentQueueRef.current.push(intent);
      if (flushTimerRef.current) window.clearTimeout(flushTimerRef.current);
      // Coalesce near-simultaneous APPLY events into one cheap reconciliation pass.
        ...
        <div className="david-app-workbench__body">
          <section className="david-app-workbench__section david-app-workbench__main-prompt">
            ...
            <div className="flex flex-wrap gap-1.5 items-center">
              ...
              <button type="button" className="david-app-workbench__button is-primary" onClick={onSynthesize} disabled={!mainPrompt.trim() || loading}>
                <Wand2 size={11} /> SYNTHESIZE THIS
              </button>
            </div>
            {lastApplied && <div className="text-[9px] opacity-50 mt-2">LAST: {lastApplied}</div>}
          </section>
          ...
          <section className="david-app-workbench__section">
            ...
            <div className="space-y-1.5">
              {(Object.keys(LOCK_LABELS) as LockKey[]).map((key) => {
                const item = locks[key];
                return (
                  <div className="david-app-workbench__lock-row" key={key}>
                    <button
                      type="button"
                      className={`david-app-workbench__lock-toggle ${item.enabled ? 'is-active' : ''}`}
                      onClick={() => setLocks((state) => ({ ...state, [key]: { ...state[key], enabled: !state[key].enabled } }))}
                      title={`${item.enabled ? 'Unlock' : 'Lock'} ${LOCK_LABELS[key]}`}
                    >
                      {item.enabled ? <Lock size={11} /> : <LockOpen size={11} />}
                    </button>
                    <span className="david-app-workbench__lock-label">{LOCK_LABELS[key]}</span>
                    <input
                      value={item.value}
                      onChange={(event) => setLocks((state) => ({ ...state, [key]: { ...state[key], value: event.target.value } }))}
                      placeholder={key === 'density' ? 'maximalist / sparse / etc.' : `lock ${key}...`}
                    />
                  </div>
                );
              })}
          ...
          <section className="david-app-workbench__section">
            ...
            <div className="space-y-1.5 text-[10px] leading-relaxed">
              {options.length ? options.slice(0, 5).map((option, index) => (
                <button key={`${option}-${index}`} type="button" className="david-app-workbench__option" onClick={() => void sendMessage(`Use this suggestion if it improves the current work: ${option}`)}>
                  <span className="opacity-45">{String(index + 1).padStart(2, '0')}</span><span>{option}</span>
                </button>
              )) : <div className="opacity-40">Ask “What are my options?” or apply a section and David will leave useful next moves here.</div>}
              <div className="border-t terminal-border pt-2 opacity-60">NEXT: {nextAction}</div>
            </div>
          </section>

          <section className="david-app-workbench__section david-app-workbench__chat">
            <div className="david-app-workbench__section-title"><span className="flex items-center gap-1.5"><Bot size={12} /> TALK TO DAVID</span><span className="opacity-45">{highThinking ? 'HIGH' : 'STD'}</span></div>
            <div className="david-app-workbench__messages">
              {messages.slice(-8).map((message, index) => (
                <div key={index} className={`david-app-workbench__message is-${message.role}`}>
                  <span>{message.role === 'david' ? 'D8' : 'YOU'}</span>
                  <p>{message.content}</p>
                </div>
              ))}
              {loading && <div className="david-app-workbench__message is-david"><span>D8</span><p>processing…</p></div>}
            </div>
            <div className="flex gap-1.5 flex-wrap mb-2">
              {['What are my options?', 'What would you change?', 'How can I fuck this up?'].map((text) => (
                <button type="button" key={text} className="david-app-workbench__chip" onClick={() => void sendMessage(text)} disabled={loading}>{text}</button>
              ))}
            </div>
            <div className="flex gap-2 items-end">
              <textarea
                ...
                rows={2}
                placeholder="Tell David what to preserve, change, add, remove, or rethink…"
                className="david-app-workbench__chat-input"
              />
              <button type="button" className="david-app-workbench__send" onClick={() => void sendMessage()} disabled={!input.trim() || loading}><Send size={14} /></button>
            </div>
          </section>
